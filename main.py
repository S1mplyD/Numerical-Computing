...
def f(x):
  J = x.reshape(m,n)
  res = 0.5*(np.linalg.norm(A(J, K)-Xoriginal,ord=2))**2
  fVal[:,k] = res
  return res
...
def df(x):
  J = x.reshape(m,n)
  res = AT(A(J, K), K) - ATb
  err_norm_f[:,k]=(np.linalg.norm(J-Xoriginal))
...
def freg(x):
  J = x.reshape(m,n)
  res = 0.5*(np.linalg.norm(A(J, K)-Xoriginal))**2 + (_lambda/2)*(np.linalg.norm(J))**2
  fregVal[:,k] = res
  return res
...
def dfreg(x):
  ...
  err_norm_freg[:,k]=(np.linalg.norm(J-Xoriginal))
  RES = np.reshape(res, m*n)
...
def minimize2(x0): # funzione che implementa il metodo del gradiente
  ...
  while (np.linalg.norm(dfreg(x_last))>ABSOLUTE_STOP and k < MAXITERATIONS ):
    ...
    error_list[:,k]=np.linalg.norm(x_last-Xoriginal)
